...
Consumption is the provider's own reported usage: input, including cached input as the provider normalizes it, plus output. A report is accepted only when its counts are present and non-negative and their totals stay representable — for an OpenAI-compatible endpoint as much as for a CLI — so an absent or unusable figure is unknown rather than a measured zero. A reservation is persisted before each invocation, so a crash records an unmeasured call as *unknown* rather than as free work; the same applies to calls reported without usage, to figures that are incomplete, negative or too large to add, and to a session's history from before this accounting existed. With a token budget configured, an assignment carrying unknown calls waits for an explicit owner decision.
