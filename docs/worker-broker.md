...
The worker runs from a private durable home in application state, not the operator's own CLI home. The library owns that home's configuration and shares only the login, so a worker gets the operator's account without their settings, project instructions, plugins or unrelated MCP servers. Credentials stay in private application state and never reach the workspace, the model, a tool result, a log or an error.
