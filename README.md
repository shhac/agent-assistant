...
`0` is the default and disables the budget. Otherwise the count is the provider's own reported usage — input, which includes cached input as the provider reports it, plus output. It is a **threshold checked before the next model invocation**, not a prepaid cap: the invocation that crosses it finishes, and a CLI or provider may make more than one upstream request inside one invocation. This counts tokens, not money; no price is estimated and no pricing table is consulted.
