// Package statepath creates private daemon directories without following child symlinks.
package statepath
...
func EnsureDirectory(root string, components ...string) (string, error) {
	...
	for _, name := range components {
		...
		if runtime.GOOS == "windows" {
			// A read-only directory handle cannot change Windows attributes. Root.Chmod
			// opens the anchored directory with the required attribute access instead.
