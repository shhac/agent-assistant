...
func (s *Service) SeedDemo(ctx context.Context) error {
	return s.store.update(ctx, func(v *Snapshot) error {
		...
		now := s.now().UTC()
		p := Project{ID: "demo-project", Title: "Community knowledge hub", Description: "Make the team's useful answers easy to find.", AcceptanceCriteria: "A searchable prototype with accessible keyboard navigation and a review report.", Status: "active", UpdatedAt: now}
