...
func TestClaudeTransportIsCoordinationOnlyAndUsesState(t *testing.T) {
	...
	engine, err := New(cfg, ExecutorFunc(func(context.Context, string, json.RawMessage) (any, error) {
	...
	result, err := engine.Chat(context.Background(), Request{Message: "Hello"})
	if err != nil || result.Message != "Ready" || !result.Usage.Known || result.Usage.TotalTokens != 19 || calls != 2 {
