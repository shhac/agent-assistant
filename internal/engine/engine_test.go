...
func TestToolSchemaContainsOnlyCoordinationSurface(t *testing.T) {
	required := map[string]bool{"read_state": true, "create_project": true, "delegate": true, "ask_decision": true, "remember_preference": true, "report_status": true, "complete_project": true, "message_agent": true}
