...
func codexArgs(cfg Config, dir, catalogPath, schemaPath, instructionsPath string) []string {
	...
	args = append(args, "-c", `model_provider="assistant_codex"`, "-c", `model_providers.assistant_codex={name="Assistant Codex",requires_openai_auth=true,wire_api="responses",request_max_retries=0,stream_max_retries=0,supports_websockets=false}`)
...
func probeCodex(ctx context.Context, cfg Config, bin string, args []string, dir string, env []string) error {
	...
	provider := `model_providers.assistant_probe={name="Assistant capability check",base_url="http://` + listener.Addr().String() + `/v1",wire_api="responses",request_max_retries=0,stream_max_retries=0,env_key="ASSISTANT_PROBE_KEY"}`
	probeArgs = append(probeArgs, "-c", `model_provider="assistant_probe"`, "-c", provider)
	probeCtx, cancel := context.WithTimeout(ctx, 20*time.Second)
	defer cancel()
	_, _ = runCodex(probeCtx, cfg, bin, probeArgs, dir, append(env, "ASSISTANT_PROBE_KEY=local-dummy-value"), `{"messages":[{"role":"user","content":"Return an empty response."}],"available_tools":[]}`)
	mu.Lock()
