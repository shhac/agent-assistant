...
// are used. A CLI upgrade cannot silently turn the PA into a shell-capable agent.
func probeClaude(ctx context.Context, cfg Config, bin string, args []string, dir string, env []string, schema []byte) error {
	...
	for _, entry := range env {
		...
		if strings.HasPrefix(entry, "HOME=") || strings.HasPrefix(entry, "CLAUDE_CONFIG_DIR=") {
			continue
		}
		probeEnv = append(probeEnv, entry)
	}
	probeEnv = append(probeEnv, "HOME="+dir, "CLAUDE_CONFIG_DIR="+dir, "ANTHROPIC_API_KEY=agent-assistant-local-probe", "ANTHROPIC_BASE_URL=http://"+listener.Addr().String())
	...
	defer mu.Unlock()
	if requests != 1 || !valid {
		return errors.New("Claude CLI capability check failed; update Claude Code before using it for coordination")
	}
	return nil
}
