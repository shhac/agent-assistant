...
func runModelTransport(ctx context.Context, cfg Config, exchange func(io.Reader, io.Writer) error) error {
	...
	dir, err := os.MkdirTemp("", "agent-assistant-model-catalog-")
	if err != nil {
		return err
	}
	...
	defer process.close()
	cmd.Cancel = func() error { process.stop(); return nil }
...
func readModelCatalog(reader io.Reader, writer io.Writer) ([]ModelOption, error) {
	...
	if err := request(1, "initialize", map[string]any{"clientInfo": map[string]string{"name": "agent-assistant", "version": "1"}}); err != nil {
		return nil, err
	}
